...
import { Quote, Star } from "@/components/ds/icons";

// PLACEHOLDER — confirm with client: all testimonials, names and roles are invented.
const quotes = [
  {
  ...
  return (
    <section id="results" style={{ background: "var(--bg-page)", padding: "84px 0" }}>
      <div style={{ maxWidth: "var(--container-max)", margin: "0 auto", padding: "0 24px" }}>
        ...
        <div className="wc-quote-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
          {quotes.map((t, i) => (
            <figure
              key={i}
              style={{
            ...
            >
              ...
              >
                ...
                <span>
                  <span style={{ display: "block", fontWeight: 600, fontSize: 14.5, color: "var(--text-strong)" }}>
                    {t.name}
                  </span>
